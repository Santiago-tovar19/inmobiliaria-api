...
{
    public function getGraduatesStudents(Request $request)
    {
    ...
    public function getAllGraduates(Request $request, $fromController=false)
    {
        ...
        $newStudents = $tudents->map(function($item) use ($request){
            ...
            $newItem->{'Carrera'} = $item->collegeDegrees->where('pivot.status_id', 3)->when(($request->input('college_degree_id')!=''), function($q) use($request){
    ...
    public function exportPDFGraduates(Request $request)
    {

        $data = [
            'students' => self::getAllGraduates($request, true),
            'type' => 'Graduados'
        ];
    ...
    public function show($id)
    {
        ...
        $c = $user->collegeDegrees->where('pivot.status_id', 3)->values()->all();
